...
def test_getModelRunID(require_model):
    """Test model registry"""
    from App.Dockers.fastapi.main import getModelRunID
    
    model_info = getModelRunID()
    assert model_info is not None
    assert hasattr(model_info, 'name')
    print(f"✅ Model: {model_info.name}")

def test_predict_returns_prediction(client, require_model):
