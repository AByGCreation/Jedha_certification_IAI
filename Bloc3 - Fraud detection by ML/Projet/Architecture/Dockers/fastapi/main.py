...
@app.post("/spredict", response_model=PredictionResponse)
async def presdict(transaction: Transaction):
    """
    Make a fraud prediction for a single transaction.
    """
    ...
    print(f"Data for prediction: {input_data.to_dict(orient='records')}")
    ...
    try:
        ...
        print(f"Data for prediction: {df.to_dict(orient='records')}")
